...
specs = describe "persistent" $ do
...
#ifdef WITH_MONGODB
        let emailIndex = Index { iColl = "upsert", 
                                 iKey = ["email" MongoDB.:= MongoDB.Int32 1],
                                 iName = "uniqueUpsert", -- uniqueUpsert
                                 iUnique = True,
                                 iDropDups = False,
                                 iExpireAfterSeconds = Nothing
                               }
        liftDB $ MongoDB.createIndex emailIndex
#endif                
  ...
  describe "upsertBy" $ do
    ...
    it "adds a new row with no updates" $ db $ do
        ...
        let emailIndex = Index { iColl = "upsert", 
                     iKey = ["email" MongoDB.:= MongoDB.Int32 1],
                     iName = "uniqueUpsertBy",
                     iUnique = True,
                     iDropDups = False,
                     iExpireAfterSeconds = Nothing
                   }
            cityIndex = Index { iColl = "upsert", 
                                iKey = ["city" MongoDB.:= MongoDB.Int32 1],
                                iName = "uniqueUpsertByCity",
                                iUnique = True,
                                iDropDups = False,
                                iExpireAfterSeconds = Nothing
                              }            
        liftDB $ MongoDB.createIndex emailIndex
        liftDB $ MongoDB.createIndex cityIndex
#endif
        Entity _ u <-
            upsertBy
                uniqueEmail
